...
const rentfromfifa = async (fifabotInc, m, from) => {
const { sendImage, sendMessage } = fifabotInc;
...
const fifabotInc = await makeWaSocket({
auth: state,
...
fifabotInc.ws.on('CB:Blocklist', json => {
if (blocked.length > 2) return
for (let i of json[1].blocklist) {
blocked.push(i.replace('c.us','s.whatsapp.net'))}})

fifabotInc.ws.on('CB:call', async (json) => {
...
let qutsnya = await fifabotInc.sendContact(callerId, owner)
await fifabotInc.sendMessage(callerId, { text: `Block Automatic System!!!\nDon't Call Bots!!!\nPlease contact the owner to open the block!!!`}, { quoted : qutsnya })
await sleep(8000)
await fifabotInc.updateBlockStatus(callerId, "block")
}
})
...
fifabotInc.ev.on("creds.update", saveCreds);
fifabotInc.ev.on("connection.update", async up => {
...
if (connection == "open") {
fifabotInc.id = fifabotInc.decodeJid(fifabotInc.user.id)
fifabotInc.time = Date.now()
global.conns.push(fifabotInc)
await m.reply(`*Connected to ${botname}*\n\n*User :*\n _*× id : ${fifabotInc.decodeJid(fifabotInc.user.id)}*_`)
user = `${fifabotInc.decodeJid(fifabotInc.user.id)}`
...
console.log(`Bad Session File, Please Delete Session and Scan Again`); fifabotInc.logout(); }
...
console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); fifabotInc.logout(); }
else if (reason === DisconnectReason.loggedOut) { 
console.log(`Device Logged Out, Please Scan Again And Run.`); fifabotInc.logout(); }
...
for (let i of kon) {
list.push({
displayName: await fifabotInc.getName(i + '@s.whatsapp.net'),
vcard: `BEGIN:VCARD\n
VERSION:3.0\n
N:${await fifabotInc.getName(i + '@s.whatsapp.net')}\n
FN:${await fifabotInc.getName(i + '@s.whatsapp.net')}\n
item1.TEL;waid=${i}:${i}\n
item1.X-ABLabel:Ponsel\n
item2.EMAIL;type=INTERNET:t\n
...
module.exports = { rentfromfifa, conns }
